...
DEFAULT_CONFIG = {
    ...
    "PANEL_PASS": "",
    "SERVICES": [
        # Example service with new fields
        {
    ...
    "START_MESSAGE_MY": (
        "🌟 *V2RAY X-UI PANEL မှ ကြိုဆိုပါတယ်* 🌟\n\n"
        "🇹🇭 AIS / TRUE နှင့် 🇲🇲 Myanmar All Sim Wifi\n"
        "အင်တာနက် ဝန်ဆောင်မှုများကို အခုပဲ စတင်သုံးစွဲလိုက်ပါ။\n\n"
        "📌 *အောက်ပါ ခလုတ်များမှ ဝန်ဆောင်မှုများကို ရွေးချယ်ပါ။*"
    ),
    "START_MESSAGE_EN": "🌟 *Welcome to V2RAY X-UI PANEL* 🌟\n\nChoose a service below to get started.",
    "START_MESSAGE_TH": "🌟 *ยินดีต้อนรับสู่ V2RAY X-UI PANEL* 🌟\n\nเลือกบริการด้านล่างเพื่อเริ่มต้น",
...
def normalize_services():
    """Ensure every service has the new fields with defaults."""
    for svc in CONFIG.get("SERVICES", []):
        svc.setdefault("security", "none")
        svc.setdefault("encryption", "none")
        svc.setdefault("flow", "")
        svc.setdefault("sni", "")
...
def get_config():
    ...
    num_services = int(input("\nHow many services do you want to add? (e.g., 3): ").strip())
    services = []
    for i in range(1, num_services + 1):
        print(f"\n--- Service {i} ---")
        name = input(" Service name (e.g., 🇹🇭 AIS 64/128 KBPS { V2RAY }): ").strip()
        ...
        # New fields
        security = input(" Security (none / tls) [none]: ").strip() or "none"
        encryption = input(" Encryption (none / ...) [none]: ").strip() or "none"
        flow = input(" Flow (leave empty if none): ").strip()
        sni = input(" SNI (Server Name Indication, leave empty if none): ").strip()
        ...
        services.append({
            ...
            "sni": sni,
            "plans": [
                {"name": name.replace("{ V2RAY }", "{ 1 MONTH }"), "days": 30, "price": 30, "total_gb": 150},
                {"name": name.replace("{ V2RAY }", "{ 2 MONTHS }"), "days": 60, "price": 60, "total_gb": 300},
            ],
        })
...
TEXTS = {
    "en": {
        ...
        "enter_username": "👤 *Please send username for this config.*\n\nAllowed: `A-Z a-z 0-9 _ . -` and Burmese/Thai letters.\nExample: `mgmg123` or `ကျော်ကျော်`\n\nUse /cancel to stop.",
        "invalid_username": "❌ Invalid username.\nUse letters (English, Burmese, Thai), numbers, _ . -\nLength 3 to 32 characters.",
        ...
        "admin_edit_plans": "✏️ Manage Plans",
        "admin_edit_topups": "💰 Edit Topup Amounts",
        "admin_edit_startmsg": "✏️ Edit Start Message",
        "admin_stats": "📊 Statistics",
        "admin_user_mgmt": "👥 User Mgmt",
        "admin_edit_services": "🛠 Edit Services",
        "admin_add_service": "➕ Add Service",  # NEW
        ...
        "no_configs_user": "❌ This user has no configs.",
        "edit_plan_field": "✏️ Edit {field} for plan:\nCurrent value: `{current}`",
        "enter_new_name": "Enter new plan name:",
        "enter_new_price": "Enter new price (฿):",
        "enter_new_days": "Enter new validity days:",
        "enter_new_gb": "Enter new total GB:",
        "plan_updated": "✅ Plan updated: {field} changed to `{new}`",
        "select_plan_to_edit": "Select a plan to edit",
        "select_language_for_startmsg": "Select language for start message:",
        "enter_new_startmsg": "Send the new start message.\nYou can use Markdown.\nUse /cancel to abort.",
        "startmsg_updated": "✅ Start message for {lang} updated.",
        "edit_service_field": "✏️ Edit {field} for service:\nCurrent value: `{current}`",
        "service_updated": "✅ Service updated: {field} changed to `{new}`",
        "select_service_to_edit": "Select a service to edit",
        "service_name": "Service Name",
        "service_inbound_id": "Inbound ID",
        "service_port": "Port",
        "service_ws_path": "WS Path",
        "service_server_address": "Server Address",
        "service_ws_host": "WS Host",
        "service_security": "Security",
        "service_encryption": "Encryption",
        "service_flow": "Flow",
        "service_sni": "SNI",
        # NEW strings for adding service
        "add_service_name": "Enter service name:",
        "add_service_inbound_id": "Enter inbound ID (integer):",
        "add_service_port": "Enter port (integer):",
        "add_service_ws_path": "Enter WS path (e.g., /path):",
        "add_service_server_address": "Enter server address / domain:",
        "add_service_ws_host": "Enter WS host:",
        "add_service_security": "Enter security (none / tls):",
        "add_service_encryption": "Enter encryption (none / ...):",
        "add_service_flow": "Enter flow (optional, press Enter to skip):",
        "add_service_sni": "Enter SNI (optional, press Enter to skip):",
        "service_added": "✅ Service added successfully!",
        # Profile
        "profile_info": "👤 *Profile*\n\n🆔 ID: `{user_id}`\n📛 Username: `{username}`\n💰 Balance: `{balance} ฿`",
        "profile_unlimited": "👤 *Profile*\n\n🆔 ID: `{user_id}`\n📛 Username: `{username}`\n💰 Balance: `♾️ Unlimited`",
    },
    "my": {
        ...
        "contact_text": "📞 *ဆက်သွယ်ရန်*\n\nTelegram: {username}\n\nအောက်ပါခလုတ်ကို နှိပ်၍ ကျွန်ုပ်တို့နှင့် ဆက်သွယ်ပါ။",
        "contact_btn": "💬 ဆက်သွယ်ရန်",
        "admin_add_bank": "➕ Bank ထည့်ရန်",
        "admin_pending_topups": "📋 Pending TopUps",
        "admin_manage_banks": "🏦 Bank များစီမံရန်",
        "admin_broadcast": "📢 Broadcast",
        "admin_edit_plans": "✏️ Plan များစီမံရန်",
        "admin_edit_topups": "💰 Topup ပမာဏများပြင်ရန်",
        "admin_edit_startmsg": "✏️ ကြိုဆိုစာ ပြင်ရန်",
        "admin_stats": "📊 စာရင်းအင်း",
        "admin_user_mgmt": "👥 အသုံးပြုသူစီမံ",
        "admin_edit_services": "🛠 Service များပြင်ရန်",
        "admin_add_service": "➕ Service အသစ်ထည့်ရန်",  # NEW
        ...
        "list_all_users": "📋 အသုံးပြုသူအားလုံး (Config အသေးစိတ်)",
        ...
        "no_configs_user": "❌ ဒီအသုံးပြုသူတွင် Config မရှိပါ။",
        "edit_plan_field": "✏️ {field} ကို ပြင်ရန်:\nလက်ရှိတန်ဖိုး: `{current}`",
        "enter_new_name": "Plan အမည်အသစ် ထည့်ပါ:",
        "enter_new_price": "စျေးနှုန်းအသစ် (ဘတ်) ထည့်ပါ:",
        "enter_new_days": "သက်တမ်းရက် အသစ် ထည့်ပါ:",
        "enter_new_gb": "စုစုပေါင်း GB အသစ် ထည့်ပါ:",
        "plan_updated": "✅ Plan အသစ်ပြင်ပြီး: {field} ကို `{new}` သို့ ပြောင်းပြီး",
        "select_plan_to_edit": "ပြင်လိုသော Plan ကို ရွေးပါ",
        "select_language_for_startmsg": "ကြိုဆိုစာ ပြင်လိုသော ဘာသာစကား ရွေးပါ:",
        "enter_new_startmsg": "ကြိုဆိုစာအသစ် ပို့ပါ။\nMarkdown သုံးနိုင်သည်။\nဖျက်ရန် /cancel နှိပ်ပါ။",
        "startmsg_updated": "✅ {lang} ဘာသာဖြင့် ကြိုဆိုစာ အသစ်ပြင်ပြီး။",
        "edit_service_field": "✏️ Service {field} ကို ပြင်ရန်:\nလက်ရှိတန်ဖိုး: `{current}`",
        "service_updated": "✅ Service ပြင်ပြီး: {field} ကို `{new}` သို့ ပြောင်းပြီး",
        "select_service_to_edit": "ပြင်လိုသော Service ကို ရွေးပါ",
        "service_name": "Service အမည်",
        "service_inbound_id": "Inbound ID",
        "service_port": "Port",
        "service_ws_path": "WS Path",
        "service_server_address": "Server Address",
        ...
        "service_sni": "SNI",
        # NEW strings for adding service
        "add_service_name": "Service အမည်ထည့်ပါ:",
        "add_service_inbound_id": "Inbound ID (ကိန်းပြည့်) ထည့်ပါ:",
        "add_service_port": "Port (ကိန်းပြည့်) ထည့်ပါ:",
        "add_service_ws_path": "WS path (e.g., /path) ထည့်ပါ:",
        "add_service_server_address": "Server address / domain ထည့်ပါ:",
        "add_service_ws_host": "WS host ထည့်ပါ:",
        "add_service_security": "Security (none / tls) ထည့်ပါ:",
        "add_service_encryption": "Encryption (none / ...) ထည့်ပါ:",
        "add_service_flow": "Flow (မရှိပါက Enter နှိပ်ပါ):",
        "add_service_sni": "SNI (မရှိပါက Enter နှိပ်ပါ):",
        "service_added": "✅ Service အသစ်ထည့်ပြီးပါပြီ။",
        "profile_info": "👤 *ပရိုဖိုင်*\n\n🆔 ID: `{user_id}`\n📛 Username: `{username}`\n💰 Balance: `{balance} ฿`",
        "profile_unlimited": "👤 *ပရိုဖိုင်*\n\n🆔 ID: `{user_id}`\n📛 Username: `{username}`\n💰 Balance: `♾️ အကန့်အသတ်မရှိ`",
    },
    "th": {
        ...
        "enter_username": "👤 *ส่ง username* (อังกฤษ, พม่า, ไทย, ตัวเลข, _ . -)\nยกเลิก /cancel",
        "invalid_username": "❌ Username ไม่ถูกต้อง\nใช้อักษรอังกฤษ พม่า ไทย ตัวเลข และ _ . - เท่านั้น\nความยาว 3-32 ตัวอักษร",
        ...
        "admin_edit_plans": "✏️ จัดการแพ็กเกจ",
        "admin_edit_topups": "💰 แก้ไขจำนวนเงินเติม",
        "admin_edit_startmsg": "✏️ แก้ไขข้อความต้อนรับ",
        "admin_stats": "สถิติ",
        "admin_user_mgmt": "จัดการผู้ใช้",
        "admin_edit_services": "🛠 แก้ไขบริการ",
        "admin_add_service": "➕ เพิ่มบริการ",  # NEW
        ...
        "no_configs_user": "ผู้ใช้รายนี้ไม่มี Config",
        "edit_plan_field": "✏️ แก้ไข {field} ของแพ็กเกจ:\nค่าปัจจุบัน: `{current}`",
        "enter_new_name": "ป้อนชื่อแพ็กเกจใหม่:",
        "enter_new_price": "ป้อนราคาใหม่ (บาท):",
        "enter_new_days": "ป้อนวันหมดอายุใหม่:",
        "enter_new_gb": "ป้อน GB ใหม่:",
        "plan_updated": "✅ อัปเดตแพ็กเกจ: {field} เปลี่ยนเป็น `{new}`",
        "select_plan_to_edit": "เลือกแพ็กเกจที่ต้องการแก้ไข",
        "select_language_for_startmsg": "เลือกภาษาสำหรับข้อความต้อนรับ:",
        "enter_new_startmsg": "ส่งข้อความต้อนรับใหม่\nสามารถใช้ Markdown ได้\nยกเลิก /cancel",
        "startmsg_updated": "✅ อัปเดตข้อความต้อนรับสำหรับ {lang} แล้ว",
        "edit_service_field": "✏️ แก้ไข {field} ของบริการ:\nค่าปัจจุบัน: `{current}`",
        "service_updated": "✅ อัปเดตบริการ: {field} เปลี่ยนเป็น `{new}`",
        "select_service_to_edit": "เลือกบริการที่ต้องการแก้ไข",
        ...
        "service_sni": "SNI",
        # NEW strings for adding service
        "add_service_name": "ป้อนชื่อบริการ:",
        "add_service_inbound_id": "ป้อน Inbound ID (จำนวนเต็ม):",
        "add_service_port": "ป้อน Port (จำนวนเต็ม):",
        "add_service_ws_path": "ป้อน WS path (เช่น /path):",
        "add_service_server_address": "ป้อน Server address / domain:",
        "add_service_ws_host": "ป้อน WS host:",
        "add_service_security": "ป้อน Security (none / tls):",
        "add_service_encryption": "ป้อน Encryption (none / ...):",
        "add_service_flow": "ป้อน Flow (ถ้าไม่มี กด Enter):",
        "add_service_sni": "ป้อน SNI (ถ้าไม่มี กด Enter):",
        "service_added": "✅ เพิ่มบริการแล้ว!",
        "profile_info": "👤 *โปรไฟล์*\n\n🆔 ID: `{user_id}`\n📛 Username: `{username}`\n💰 Balance: `{balance} ฿`",
        "profile_unlimited": "👤 *โปรไฟล์*\n\n🆔 ID: `{user_id}`\n📛 Username: `{username}`\n💰 Balance: `♾️ ไม่จำกัด`",
    },
}
...
class XUIClient:
    ...
    def add_client(self, inbound_id, email, uuid_str, total_gb=0, expiry_time=0):
        ...
        endpoints = [
            ...
            f"{self.base_url}/server/api/inbounds/addClient",
        ]
        last_exception = None
        for url in endpoints:
            try:
                ...
                if resp.status_code == 200:
                    try:
                        ...
                        if result.get("success"):
                            logger.info(f"Client added using {url} (form-data)")
                            return result
                    except:
                        pass
            except Exception as e:
                ...
                continue
            try:
                json_data = {"id": inbound_id, "settings": settings}
                resp = self.session.post(url, json=json_data, timeout=20)
                if resp.status_code == 200:
                    try:
                        result = resp.json()
                        if result.get("success"):
                            logger.info(f"Client added using {url} (JSON)")
                            return result
                    except:
                        pass
            except Exception as e:
                last_exception = e
                continue
        raise Exception(f"Add client failed on all endpoints. Last error: {last_exception}")
...
def is_valid_xui_email_value(username):
    pattern = r'^[\w\u1000-\u109F\u0E00-\u0E7F_.-]{3,32}$'
    return bool(re.fullmatch(pattern, username, re.UNICODE))
...
def generate_vless_link(uuid_str, remark, service_config):
    ...
    security = service_config.get("security", "none")
    encryption = service_config.get("encryption", "none")
    flow = service_config.get("flow", "")
    sni = service_config.get("sni", "")
    ...
    link = f"vless://{uuid_str}@{address}:{port}?{param_str}"
    if remark:
...
def build_client_status_text(client, traffic, lang, online_emails=None):
    ...
    if not enabled or is_expired or traffic_exhausted:
        if not enabled:
...
async def get_main_keyboard(is_admin, lang):
    buttons = [
        ...
        [KeyboardButton(get_text("profile", lang)), KeyboardButton(get_text("contact", lang))],  # PROFILE added
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    username_display = user.username or user.full_name or str(user_id)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Broadcast state (handles all message types) ---
    if state == "broadcast":
        await handle_broadcast_message(update, context)
        return

    # --- Add service state ---
    if state == "add_service_name":
        context.user_data["add_service_name"] = text
        context.user_data["state"] = "add_service_inbound_id"
        await update.message.reply_text(get_text("add_service_inbound_id", lang))
        return
    if state == "add_service_inbound_id":
        try:
            context.user_data["add_service_inbound_id"] = int(text)
        except ValueError:
            await update.message.reply_text("❌ Invalid inbound ID. Enter an integer.")
            return
        context.user_data["state"] = "add_service_port"
        await update.message.reply_text(get_text("add_service_port", lang))
        return
    if state == "add_service_port":
        try:
            context.user_data["add_service_port"] = int(text)
        except ValueError:
            await update.message.reply_text("❌ Invalid port. Enter an integer.")
            return
        context.user_data["state"] = "add_service_ws_path"
        await update.message.reply_text(get_text("add_service_ws_path", lang))
        return
    if state == "add_service_ws_path":
        context.user_data["add_service_ws_path"] = text or "/"
        context.user_data["state"] = "add_service_server_address"
        await update.message.reply_text(get_text("add_service_server_address", lang))
        return
    if state == "add_service_server_address":
        context.user_data["add_service_server_address"] = text
        context.user_data["state"] = "add_service_ws_host"
        await update.message.reply_text(get_text("add_service_ws_host", lang))
        return
    if state == "add_service_ws_host":
        context.user_data["add_service_ws_host"] = text
        context.user_data["state"] = "add_service_security"
        await update.message.reply_text(get_text("add_service_security", lang))
        return
    if state == "add_service_security":
        context.user_data["add_service_security"] = text or "none"
        context.user_data["state"] = "add_service_encryption"
        await update.message.reply_text(get_text("add_service_encryption", lang))
        return
    if state == "add_service_encryption":
        context.user_data["add_service_encryption"] = text or "none"
        context.user_data["state"] = "add_service_flow"
        await update.message.reply_text(get_text("add_service_flow", lang))
        return
    if state == "add_service_flow":
        context.user_data["add_service_flow"] = text
        context.user_data["state"] = "add_service_sni"
        await update.message.reply_text(get_text("add_service_sni", lang))
        return
    if state == "add_service_sni":
        context.user_data["add_service_sni"] = text
        # Build and save the new service
        new_service = {
            "name": context.user_data["add_service_name"],
            "inbound_id": context.user_data["add_service_inbound_id"],
            "port": context.user_data["add_service_port"],
            "ws_path": context.user_data["add_service_ws_path"],
            "server_address": context.user_data["add_service_server_address"],
            "ws_host": context.user_data["add_service_ws_host"],
            "security": context.user_data["add_service_security"],
            "encryption": context.user_data["add_service_encryption"],
            "flow": context.user_data["add_service_flow"],
            "sni": context.user_data["add_service_sni"],
            "plans": [
                {"name": f"{context.user_data['add_service_name']} {{ 1 MONTH }}", "days": 30, "price": 30, "total_gb": 150},
                {"name": f"{context.user_data['add_service_name']} {{ 2 MONTHS }}", "days": 60, "price": 60, "total_gb": 300},
            ]
        }
        CONFIG["SERVICES"].append(new_service)
        save_config()
        context.user_data.clear()
        await update.message.reply_text(get_text("service_added", lang))
        await send_main_menu_by_message(update.message, user_id, lang)
        return

    # Other states (buy, admin note, etc.)
    ...
    if state == "edit_bank_qr":
        ...
        return
    # Plan editing states
    if state == "edit_plan_select_service":
        await handle_edit_plan_select_service(update, context, text)
        return
    if state == "edit_plan_select_plan":
        await handle_edit_plan_select_plan(update, context, text)
        return
    if state == "edit_plan_field":
        await handle_edit_plan_field(update, context, text)
        return
    # Service editing states
    if state == "edit_service_select_service":
        await handle_edit_service_select_service(update, context, text)
        return
    if state == "edit_service_field":
        await handle_edit_service_field(update, context, text)
        return
    # Start message editing
    if state == "edit_startmsg_select_lang":
        await handle_edit_startmsg_select_lang(update, context, text)
        return
    if state == "edit_startmsg_enter_text":
        await handle_edit_startmsg_enter_text(update, context, text)
        return
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "edit_bank_qr":
        await handle_edit_bank_qr(update, context)
        return
    # Broadcast: photos are handled in handle_broadcast_message
    if state == "broadcast":
        await handle_broadcast_message(update, context)
        return
    user_id = update.effective_user.id
...
async def route_main_menu_text(update, context, text):
    ...
    elif text == get_text("profile", lang):  # NEW
        await show_profile(update, lang)
...
async def show_profile(update, lang):
    user_id = update.effective_user.id
    ...
    if is_admin:
        await update.message.reply_text(
            get_text("profile_unlimited", lang, user_id=user_id, username=update.effective_user.username or user_id),
            parse_mode="Markdown"
        )
    else:
        await update.message.reply_text(
            get_text("profile_info", lang, user_id=user_id, username=update.effective_user.username or user_id, balance=balance),
            parse_mode="Markdown"
        )
...
async def receive_slip(update, context):
    ...
    mention = f"{update.effective_user.full_name}"
    try:
        await context.bot.send_photo(
...
async def show_admin_panel(update, lang):
    keyboard = [
        ...
        [InlineKeyboardButton(get_text("admin_edit_services", lang), callback_data="admin_edit_services")],
        [InlineKeyboardButton(get_text("admin_add_service", lang), callback_data="admin_add_service")],  # NEW
        [InlineKeyboardButton(get_text("admin_edit_topups", lang), callback_data="admin_edit_topups")],
        [InlineKeyboardButton(get_text("admin_edit_startmsg", lang), callback_data="admin_edit_startmsg")],
...
# ---------- Manage Services ----------
async def admin_edit_services(query, lang):
    services = CONFIG.get("SERVICES", [])
    keyboard = []
    for idx, svc in enumerate(services):
        keyboard.append([InlineKeyboardButton(svc["name"], callback_data=f"editsvc_{idx}")])
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="admin_panel")])
    await query.message.reply_text(get_text("select_service_to_edit", lang), reply_markup=InlineKeyboardMarkup(keyboard))

async def admin_edit_service_select(query, context, service_idx):
    user_id = query.from_user.id
    lang = await db.get_user_lang(user_id)
    ...
    if not service:
        ...
        return
    context.user_data["edit_service_idx"] = int(service_idx)
    fields = [
        ("service_name", "name"),
        ("service_inbound_id", "inbound_id"),
        ("service_port", "port"),
        ("service_ws_path", "ws_path"),
        ("service_server_address", "server_address"),
        ("service_ws_host", "ws_host"),
        ("service_security", "security"),
        ("service_encryption", "encryption"),
        ("service_flow", "flow"),
        ("service_sni", "sni"),
    ...
    keyboard = []
    for display_key, field_key in fields:
        keyboard.append([InlineKeyboardButton(get_text(display_key, lang), callback_data=f"editsvc_field_{field_key}")])
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="admin_panel")])
    await query.message.reply_text(
        f"Editing service: {service['name']}\nChoose field to edit:",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

async def admin_edit_service_field(query, context, field):
    user_id = query.from_user.id
    lang = await db.get_user_lang(user_id)
    service_idx = context.user_data.get("edit_service_idx")
    ...
    if not service:
        await query.answer("Service not found")
        return
    current = service.get(field, "")
    context.user_data["edit_service_field"] = field
    context.user_data["state"] = "edit_service_field"
    await query.message.reply_text(
        get_text("edit_service_field", lang, field=get_text(f"service_{field}", lang), current=current)
    )
    await query.message.reply_text(f"Enter new value for {field}:")

async def handle_edit_service_field(update, context, text):
    user_id = update.effective_user.id
    lang = await db.get_user_lang(user_id)
    service_idx = context.user_data.get("edit_service_idx")
    field = context.user_data.get("edit_service_field")
    if service_idx is None or field is None:
        context.user_data.clear()
    ...
    try:
        if field in ("inbound_id", "port"):
            new_value = int(new_value)
        service[field] = new_value
        save_config()
        await update.message.reply_text(get_text("service_updated", lang, field=get_text(f"service_{field}", lang), new=new_value))
    except ValueError:
        await update.message.reply_text(f"❌ Invalid value for {field}. Please enter a number.")
        return
    except Exception as e:
...
# ---------- Add Service (NEW) ----------
async def admin_add_service(query, lang):
    user_id = query.from_user.id
    context = query.bot._context  # not ideal; better to pass context via callback
    # We'll handle via callback, but we need context. Use query.message.reply_text and set state.
    await query.answer()
    context.user_data.clear()
    context.user_data["state"] = "add_service_name"
    await query.message.reply_text(get_text("add_service_name", lang))
...
async def admin_edit_plans(query, lang):
    ...
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="admin_panel")])
    await query.message.reply_text("Select service to manage its plans:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def admin_edit_plans_select_service(query, context, service_idx):
    ...
    if not service:
        await query.answer("Service not found")
        return
    context.user_data["edit_plan_service_idx"] = int(service_idx)
    keyboard = []
    for plan_idx, plan in enumerate(service["plans"]):
        keyboard.append([InlineKeyboardButton(f"{plan['name']} (Price: {plan['price']}฿, GB: {plan['total_gb']})", callback_data=f"editplan_select_plan_{plan_idx}")])
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="admin_panel")])
    await query.message.reply_text("Select a plan to edit:", reply_markup=InlineKeyboardMarkup(keyboard))

async def admin_edit_plan_select_plan(query, context, plan_idx):
    user_id = query.from_user.id
    lang = await db.get_user_lang(user_id)
    service_idx = context.user_data.get("edit_plan_service_idx")
    if service_idx is None:
        await query.answer("Session expired")
        return
    service = get_service_by_index(service_idx)
    if not service or plan_idx >= len(service["plans"]):
        ...
        return
    context.user_data["edit_plan_plan_idx"] = plan_idx
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("✏️ Name", callback_data="editplan_field_name")],
        [InlineKeyboardButton("💰 Price", callback_data="editplan_field_price")],
        [InlineKeyboardButton("📆 Days", callback_data="editplan_field_days")],
        [InlineKeyboardButton("💾 Total GB", callback_data="editplan_field_total_gb")],
        [InlineKeyboardButton("🔙 Back", callback_data="admin_panel")],
    ])
    await query.message.reply_text(f"Editing: {service['plans'][plan_idx]['name']}\nChoose what to edit:", reply_markup=keyboard)

async def admin_edit_plan_field(query, context, field):
    user_id = query.from_user.id
    lang = await db.get_user_lang(user_id)
    service_idx = context.user_data.get("edit_plan_service_idx")
    plan_idx = context.user_data.get("edit_plan_plan_idx")
    ...
    context.user_data["state"] = "edit_plan_field"
    await query.message.reply_text(
        get_text("edit_plan_field", lang, field=field.capitalize(), current=current)
    )
    prompts = {
        "name": get_text("enter_new_name", lang),
        "price": get_text("enter_new_price", lang),
        "days": get_text("enter_new_days", lang),
        "total_gb": get_text("enter_new_gb", lang),
    }
    await query.message.reply_text(prompts.get(field, "Enter new value:"))

async def handle_edit_plan_field(update, context, text):
    user_id = update.effective_user.id
    lang = await db.get_user_lang(user_id)
    service_idx = context.user_data.get("edit_plan_service_idx")
    plan_idx = context.user_data.get("edit_plan_plan_idx")
    field = context.user_data.get("edit_plan_field")
    if None in (service_idx, plan_idx, field):
        context.user_data.clear()
        await update.message.reply_text("Session expired.")
        await send_main_menu_by_message(update.message, user_id, lang)
        return
    ...
    try:
        if field == "name":
            ...
            new_value = int(new_value)
            if new_value <= 0:
                raise ValueError("Total GB must be positive")
            plan["total_gb"] = new_value
        else:
            raise ValueError("Unknown field")
        save_config()
        await update.message.reply_text(get_text("plan_updated", lang, field=field.capitalize(), new=new_value))
    except ValueError as ve:
        await update.message.reply_text(f"❌ Invalid value: {ve}\nPlease enter a valid positive number.")
        return
    except Exception as e:
        await update.message.reply_text(f"❌ Failed to update: {e}")
        return
...
# ---------- Edit Start Message ----------
async def admin_edit_startmsg(query, lang):
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🇲🇲 Myanmar", callback_data="editstart_my")],
        [InlineKeyboardButton("🇬🇧 English", callback_data="editstart_en")],
        [InlineKeyboardButton("🇹🇭 Thai", callback_data="editstart_th")],
        [InlineKeyboardButton("🔙 Back", callback_data="admin_panel")],
    ])
    await query.message.reply_text(get_text("select_language_for_startmsg", lang), reply_markup=keyboard)

async def admin_edit_startmsg_lang(query, context, lang_code):
    user_id = query.from_user.id
    lang_user = await db.get_user_lang(user_id)
    context.user_data["state"] = "edit_startmsg_enter_text"
    context.user_data["edit_startmsg_lang"] = lang_code
    await query.message.reply_text(
        get_text("enter_new_startmsg", lang_user) + f"\n\nCurrent message:\n{get_welcome_text(lang_code)}"
    )

async def handle_edit_startmsg_enter_text(update, context, text):
    user_id = update.effective_user.id
    lang = await db.get_user_lang(user_id)
    if not await db.is_admin(user_id):
        await update.message.reply_text("Admin only.")
        return
    lang_code = context.user_data.get("edit_startmsg_lang")
    if not lang_code:
        context.user_data.clear()
        await update.message.reply_text("Session expired.")
        return
    if lang_code == "my":
        CONFIG["START_MESSAGE_MY"] = text
    elif lang_code == "en":
        CONFIG["START_MESSAGE_EN"] = text
    elif lang_code == "th":
        CONFIG["START_MESSAGE_TH"] = text
    else:
        await update.message.reply_text("Invalid language.")
        return
    save_config()
    await update.message.reply_text(get_text("startmsg_updated", lang, lang=lang_code.upper()))
    context.user_data.clear()
    await send_main_menu_by_message(update.message, user_id, lang)
...
async def handle_broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles text, photo, document, etc. for broadcast."""
    user_id = update.effective_user.id
    if not await db.is_admin(user_id):
        context.user_data.clear()
        await update.message.reply_text("❌ Admin only.")
    ...
    # We'll send the same message type to each user
    for user in users:
        try:
            if update.message.text:
                await context.bot.send_message(user["user_id"], update.message.text)
            elif update.message.photo:
                await context.bot.send_photo(user["user_id"], update.message.photo[-1].file_id, caption=update.message.caption)
            elif update.message.document:
                await context.bot.send_document(user["user_id"], update.message.document.file_id, caption=update.message.caption)
            elif update.message.video:
                await context.bot.send_video(user["user_id"], update.message.video.file_id, caption=update.message.caption)
            elif update.message.audio:
                await context.bot.send_audio(user["user_id"], update.message.audio.file_id, caption=update.message.caption)
            elif update.message.voice:
                await context.bot.send_voice(user["user_id"], update.message.voice.file_id)
            elif update.message.sticker:
                await context.bot.send_sticker(user["user_id"], update.message.sticker.file_id)
            else:
                # Fallback: send as text
                await context.bot.send_message(user["user_id"], "Broadcast message (unsupported type)")
            sent += 1
        except Exception as e:
...
async def admin_stats_menu(query, lang):
    ...
    total_offline = len([c for c in active_configs if c["email"].lower() not in (online_emails or set())])
...
async def admin_list_all_users(query, lang):
    ...
    for user in users:
        ...
        configs = await db.get_clients(user_id)
        if not configs:
            await query.message.reply_text(
                f"👤 *User ID:* `{user_id}` | 💰 Balance: `{balance} ฿`\n❌ {get_text('no_configs_user', lang)}",
                parse_mode="Markdown",
            )
            continue
        await query.message.reply_text(
        ...
        for idx, client in enumerate(configs, start=1):
            traffic = xui_client.get_client_traffic(client["email"]) if xui_client else {}
            down = int(traffic.get("downlink", client.get("download_used", 0)) or 0)
            up = int(traffic.get("uplink", client.get("upload_used", 0)) or 0)
            await db.update_client_usage_by_email(client["email"], down, up)
            ...
            try:
                ...
                logger.warning(f"Failed to send QR/link for {client['email']}: {e}")
...
async def admin_edit_topups(query, lang):
    ...
    await query.message.reply_text(
        f"Current top-up amounts: {', '.join(str(a) for a in current)}\n\nSend new amounts separated by spaces or commas.\nExample: `30 60 100 200 500`\nUse /cancel to abort.",
        parse_mode="Markdown",
    )
    # State already set in callback
...
async def handle_edit_topup_amounts(update, context, text):
    ...
    if len(amounts) < 1:
        await update.message.reply_text("❌ Invalid. Send numbers separated by spaces or commas.")
        return
    CONFIG["TOPUP_AMOUNTS"] = amounts
    save_config()
    await update.message.reply_text(f"✅ Top-up amounts updated: {', '.join(str(a) for a in amounts)}")
    context.user_data.clear()
    await send_main_menu_by_message(update.message, user_id, lang)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("editplan_select_plan_"):
        plan_idx = int(data.split("_")[-1])
        await admin_edit_plan_select_plan(query, context, plan_idx)
        return

    if data.startswith("editplan_field_"):
        field = data[len("editplan_field_"):]
        await admin_edit_plan_field(query, context, field)
        return

    if data == "admin_edit_services":
        await admin_edit_services(query, lang)
        return

    if data.startswith("editsvc_field_"):
        field = data[len("editsvc_field_"):]
        await admin_edit_service_field(query, context, field)
        return

    if data.startswith("editsvc_"):
        svc_idx = data.split("_")[1]
        await admin_edit_service_select(query, context, svc_idx)
        return

    if data == "admin_add_service":
        await admin_add_service(query, lang)
        return

    if data == "admin_edit_startmsg":
        await admin_edit_startmsg(query, lang)
        return

    if data.startswith("editstart_"):
        lang_code = data.split("_")[1]
        await admin_edit_startmsg_lang(query, context, lang_code)
        return
