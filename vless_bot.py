...
def get_config():
    ...
    print("\nNow configure your services (you can add more later via Admin Panel).")
    services = []
    while True:
        print("\n--- Add a new service ---")
        name = input("Service name (or press Enter to finish): ").strip()
        if not name:
            break
        ...
        tls_input = input(" Enable TLS? (y/n, default n): ").strip().lower()
        tls = tls_input == "y"
        plans = [
            {"name": f"{name} {{ 1 MONTH }}", "days": 30, "price": 30, "total_gb": 150},
            {"name": f"{name} {{ 2 MONTHS }}", "days": 60, "price": 60, "total_gb": 300},
        ]
        services.append({
            "name": name,
...
TEXTS = {
    "en": {
        ...
        "broadcast_prompt": "📢 Send message to broadcast (text, photo, video, document, etc.).\nUse /cancel to stop.",
        ...
        "add_service_inbound_id": "Enter inbound ID:",
        "add_service_port": "Enter port:",
        "add_service_ws_path": "Enter WS Path (default /):",
        "add_service_server_address": "Enter server address/domain:",
        "add_service_ws_host": "Enter WS host:",
        "add_service_tls": "Enable TLS? (yes/no, default no):",
        "add_service_confirm": "✅ Service added successfully.",
        "add_service_cancel": "❌ Service addition cancelled.",
    },
    "my": {
        ...
        "admin_add_service": "➕ Service အသစ်ထည့်ရန်",
        ...
        "broadcast_prompt": "📢 User အားလုံးသို့ ပို့မည့် message (စာသား၊ ဓာတ်ပုံ၊ ဗီဒီယို၊ ဖိုင်) ပို့ပါ။\n/cancel ဖြင့် ရပ်နိုင်သည်။",
        "broadcast_sending": "⏳ Broadcasting...",
        ...
        "add_service_name": "Service အမည် ထည့်ပါ:",
        "add_service_inbound_id": "Inbound ID ထည့်ပါ:",
        "add_service_port": "Port ထည့်ပါ:",
        "add_service_ws_path": "WS Path (default /) ထည့်ပါ:",
        "add_service_server_address": "Server Address/Domain ထည့်ပါ:",
        "add_service_ws_host": "WS Host ထည့်ပါ:",
        "add_service_tls": "TLS ဖွင့်မလား? (yes/no, default no):",
        "add_service_confirm": "✅ Service အသစ်ထည့်ပြီးပါပြီ။",
        "add_service_cancel": "❌ Service ထည့်ခြင်းကို ပယ်ဖျက်လိုက်ပါသည်။",
    },
    "th": {
        ...
        "broadcast_prompt": "ส่งข้อความประชาสัมพันธ์ (ข้อความ, รูป, วิดีโอ, ไฟล์)",
        ...
        "service_ws_host": "WS Host",
        "service_tls": "TLS",
        "profile_text": "👤 *โปรไฟล์ของคุณ*\n\n🆔 User ID: `{user_id}`\n👤 Username: {username}\n💰 ยอดเงิน: `{balance} ฿`",
        "add_service_name": "ป้อนชื่อบริการ:",
        "add_service_inbound_id": "ป้อน Inbound ID:",
        "add_service_port": "ป้อน Port:",
        "add_service_ws_path": "ป้อน WS Path (ค่าเริ่มต้น /):",
        "add_service_server_address": "ป้อน Server Address/Domain:",
        "add_service_ws_host": "ป้อน WS Host:",
        "add_service_tls": "เปิด TLS? (yes/no, ค่าเริ่มต้น no):",
        "add_service_confirm": "✅ เพิ่มบริการสำเร็จ",
        "add_service_cancel": "❌ ยกเลิกการเพิ่มบริการ",
    }
}
...
class XUIClient:
    ...
    # ========== FIXED add_client to support both X-UI and 3x-UI ==========
...
def is_valid_xui_email_value(username):
    # Allow Unicode letters (including Burmese, Thai), numbers, underscore, dot, hyphen
    pattern = r'^[\w\u1000-\u109F\u0E00-\u0E7F_.-]{3,32}$'
...
def generate_vless_link(uuid_str, remark, service_config):
    ...
    tls = service_config.get("tls", False)
    ...
    if tls:
        ...
        params.append(f"sni={quote(address, safe='')}")
        # You may also add allowInsecure=false if needed; not adding by default.
    else:
        params.append("security=none")

    link = f"vless://{uuid_str}@{address}:{port}?" + "&".join(params)
    if remark:
...
def build_client_status_text(client, traffic, lang, online_emails=None):
    ...
    if not enabled or is_expired or traffic_exhausted:
        status_text = "Disabled" if not enabled else ("Expired" if is_expired else "Traffic Finished")
        status_emoji = "🔴" if (not enabled or is_expired) else "🔴"
        if is_expired:
            status_emoji = "🟡"
    else:
...
async def get_main_keyboard(is_admin, lang):
    buttons = [
        ...
        [KeyboardButton(get_text("profile", lang)), KeyboardButton(get_text("contact", lang))],
...
async def show_profile(update, lang):
    ...
    text = get_text("profile_text", lang, user_id=user_id, username=username, balance=balance)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Broadcast state - handle any message (text, photo, etc.)
    if state == "broadcast":
        await handle_broadcast(update, context)
        return

    # Other text-based states
    ...
    if state == "add_service_name":
        await handle_add_service(update, context, text, "name")
        return
    if state == "add_service_inbound_id":
        await handle_add_service(update, context, text, "inbound_id")
        return
    if state == "add_service_port":
        await handle_add_service(update, context, text, "port")
        return
    if state == "add_service_ws_path":
        await handle_add_service(update, context, text, "ws_path")
        return
    if state == "add_service_server_address":
        await handle_add_service(update, context, text, "server_address")
        return
    if state == "add_service_ws_host":
        await handle_add_service(update, context, text, "ws_host")
        return
    if state == "add_service_tls":
        await handle_add_service(update, context, text, "tls")
        return
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "edit_bank_qr":
        await handle_edit_bank_qr(update, context)
        return
    # If in broadcast state, handle as broadcast
    if state == "broadcast":
        await handle_broadcast(update, context)
        return
    user_id = update.effective_user.id
...
async def route_main_menu_text(update, context, text):
    ...
    elif text == get_text("profile", lang):
        await show_profile(update, lang)
...
async def show_admin_panel(update, lang):
    keyboard = [
        ...
        [InlineKeyboardButton(get_text("admin_add_service", lang), callback_data="admin_add_service")],
...
async def admin_edit_service_select(query, context, service_idx):
    ...
    # Build inline keyboard for service fields
    fields = [
        ("service_name", "name"),
...
async def admin_edit_plans_select_service(query, context, service_idx):
    ...
    for plan_idx, plan in enumerate(service["plans"]):
        keyboard.append([InlineKeyboardButton(
            f"{plan['name']} (Price: {plan['price']}฿, GB: {plan['total_gb']})",
            callback_data=f"editplan_select_plan_{plan_idx}"
        )])
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="admin_panel")])
...
async def admin_edit_plan_select_plan(query, context, plan_idx):
    ...
    await query.message.reply_text(
        f"Editing: {service['plans'][plan_idx]['name']}\nChoose what to edit:",
        reply_markup=keyboard
    )
...
async def admin_add_service(query, lang):
    user_id = query.from_user.id
    context = query._context  # hack to get context from query? Better use global context? We'll use callback context.
    # We'll set state and ask for name via callback.
...
async def handle_add_service(update, context, text, step):
    user_id = update.effective_user.id
    lang = await db.get_user_lang(user_id)
    new_service = context.user_data.get("new_service", {})
    if step == "name":
        new_service["name"] = text
        ...
        try:
            new_service["inbound_id"] = int(text)
    ...
    elif step == "server_address":
        new_service["server_address"] = text
        context.user_data["state"] = "add_service_ws_host"
        await update.message.reply_text(get_text("add_service_ws_host", lang))
    elif step == "ws_host":
        new_service["ws_host"] = text
        context.user_data["state"] = "add_service_tls"
        await update.message.reply_text(get_text("add_service_tls", lang))
    elif step == "tls":
        tls = text.lower() in ("yes", "true", "1", "y")
        new_service["tls"] = tls
        # Add default plans
        ...
        save_config()
        context.user_data.clear()
        await update.message.reply_text(get_text("add_service_confirm", lang))
        await send_main_menu_by_message(update.message, user_id, lang)
...
# ---------- Broadcast (supports media and copy button) ----------
async def handle_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Capture the message to broadcast
    msg = update.message
    if not msg:
        await update.message.reply_text("❌ No message to broadcast.")
        return

    # Store the message data in context
    ...
    # Get text and possible vless link from caption or text
    broadcast_text = msg.text or msg.caption or ""
    vless_links = re.findall(r'(vless://[^\s]+)', broadcast_text)

    for user in users:
        try:
            # Send the original message (photo, video, document, text, etc.)
            if msg.photo:
                await context.bot.send_photo(
            ...
            # If there is a VLESS link, send a copy button message
            if vless_links:
                # Send the first link with copy button
                link = vless_links[0]
                keyboard = get_vless_copy_keyboard(link, "en")  # language doesn't matter for button text
                await context.bot.send_message(
                    chat_id=user["user_id"],
                    text="📋 *Copy VLESS Link:*",
                    parse_mode="Markdown",
                    reply_markup=keyboard,
                )
...
async def admin_edit_topups(query, lang):
    ...
    context = query._context  # Use the context from callback
