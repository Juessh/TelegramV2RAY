...
DEFAULT_CONFIG = {
    ...
    "PANEL_PASS": "",
    "SERVICES": [
        # example structure – will be overwritten during setup
    ],
...
def get_config():
    ...
    num_services = int(input("How many services do you want to add? (number): ").strip())
    services = []
    for i in range(1, num_services + 1):
        ...
        # optional advanced fields – can be edited later in config.json
        security = input(" Security (none/tls, default none): ").strip() or "none"
        encryption = input(" Encryption (none, default none): ").strip() or "none"
        sni = input(" SNI (optional, press Enter to skip): ").strip()
        fp = input(" Fingerprint (optional, press Enter to skip): ").strip()
...
TEXTS = {
    "en": {
        ...
        "broadcast_prompt": "📢 Send message (text, photo, video, document, etc.) to broadcast.\nUse /cancel to stop.",
    ...
    "my": {
        ...
        "broadcast_prompt": "📢 User အားလုံးသို့ ပို့မည့် message (စာ၊ ဓာတ်ပုံ၊ ဖိုင်) ပို့ပါ။\n/cancel ဖြင့် ရပ်နိုင်သည်။",
        "broadcast_sending": "⏳ Broadcasting...",
    ...
    "th": {
        ...
        "broadcast_prompt": "ส่งข้อความประชาสัมพันธ์ (ข้อความ, รูป, ไฟล์)",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    profile_text = get_text("profile_info", lang, user_id=user_id, username=username_display, balance=balance)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Broadcast state – handle any message
    if state == "broadcast":
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "edit_bank_qr":
        await handle_edit_bank_qr(update, context)
        return
    # If broadcast state, handle as broadcast
    if state == "broadcast":
        await handle_broadcast_message(update, context)
        return
    user_id = update.effective_user.id
...
async def show_profile(update, lang):
    ...
    profile_text = get_text("profile_info", lang, user_id=user_id, username=username, balance=balance)
    await update.message.reply_text(profile_text, parse_mode="Markdown")
...
async def handle_broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(get_text("broadcast_sending", lang))

    sent = 0
    failed = 0
    # Get the message object to broadcast
    msg = update.message
    ...
    for user in users:
        try:
            ...
            if msg.text:
                # Text message – optionally add copy button if vless:// is in text
                text = msg.text
                if "vless://" in text:
                    # Extract the first vless link (simple approach)
                    import re
                ...
                await context.bot.send_photo(
                    ...
                    caption=msg.caption,
                    parse_mode=msg.caption_entities,
                )
            elif msg.document:
    ...
    context.user_data.clear()
    await update.message.reply_text(
        get_text("broadcast_result", lang, sent=sent, failed=failed)
    )
    await send_main_menu_by_message(update.message, user_id, lang)
...
async def admin_edit_service_select(query, context, service_idx):
    ...
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="admin_panel")])
    await query.message.reply_text(f"Editing service: {service['name']}\nChoose field to edit:", reply_markup=InlineKeyboardMarkup(keyboard))
